import { Input } from "../components/Input"; // input reutilizável
...
import { useNavigate } from "react-router"; // hook de navegação
import { AxiosError } from "axios"; // tipo de erro do axios

// esquema de validação para o formulário de cadastro com verificação de confirmação
...
export function SignUp() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await api.post("/users", data); // enviar dados ao backend para criar usuário

      if (confirm("Cadastrado com sucesso! Ir para o login?")) {
        navigate("/"); // se o usuário concordar, ir para a página de login
      }
    } catch (error) {
      console.log(error);

      if (error instanceof ZodError) {
        return alert(error.issues[0].message); // mostra o primeiro erro de validação
      }

      if (error instanceof AxiosError) {
        return alert(error.response?.data.message); // mostra mensagem de erro do servidor
      }
